...
export async function buscarPrecoProduto(codProd: string): Promise<number> {
  ...
  try {
    ...
    // A API retorna uma lista de itens com preços
    let preco = 0;
    
    if (resposta.data && resposta.data.items && Array.isArray(resposta.data.items) && resposta.data.items.length > 0) {
      // Pegar o primeiro item da lista
      const item = resposta.data.items[0];
      preco = item.preco || item.VLRVENDA || item.VLRUNIT || 0;
    }
    
    console.log(`💰 Preço do produto ${codProd}:`, preco);
...
export async function consultarProdutos(page: number = 1, pageSize: number = 50, searchName: string = '', searchCode: string = '') {
  ...
  try {
    ...
    // Buscar estoque e preço de cada produto de forma sequencial
    console.log(`📦 Buscando estoque e preços de ${listaProdutosLimpa.length} produtos encontrados...`);
    const produtosComEstoque = [];
    
    for (const produto of listaProdutosLimpa) {
      try {
        // Delay entre requisições para evitar concorrência
        await new Promise(resolve => setTimeout(resolve, 100));
        
        // Buscar estoque
        const resultadoEstoque = await consultarEstoqueProduto(produto.CODPROD);
        
        // Delay antes de buscar preço
        await new Promise(resolve => setTimeout(resolve, 100));
        
        // Buscar preço
        const preco = await buscarPrecoProduto(produto.CODPROD);
        
        produtosComEstoque.push({
          ...produto,
          ESTOQUE: String(resultadoEstoque.estoqueTotal || 0),
          VLRCOMERC: preco > 0 ? String(preco) : produto.VLRCOMERC
        });
